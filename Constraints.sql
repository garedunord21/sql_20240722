...
-- UNIQUE 제약조건 : 해당 칼럼에 중복된 데이터를 포함할 수 없도록 하는 제약
-- (자기 자신 테이블 INSERT, UPDATE)
...
-- UNIQUE로 지정된 컬럼은 중복된 데이터를 삽일할 수 없음
INSERT INTO unique_table VALUES(1, 1);
...
-- FOREIGN KEY 제약조건을 특정 컬럼에 적용할 때에는 반드시 데이터 타입이 참조하고자하는 컬럼의 타입과 일치해야함
...
MODIFY COLUMN primary_colunm VARCHAR(10);
...
CREATE TABLE default_table (
	-- AUTO_INCREMENT : 기본키가 정수형일때 기본키의 값을 1씩 증가하는 값으로 자동 지정
